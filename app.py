...
# --- SECRET BRIDGE (ADMIN ONLY) ---
# We ONLY load Admin login details from secrets. 
# We do NOT load OpenAI keys here to prevent users from using yours.
try:
...
<style>
    ...
    /* Nav */
    .nav-container { display: flex; justify-content: space-between; align-items: center; padding: 15px 40px; background: rgba(15, 23, 42, 0.9); backdrop-filter: blur(12px); position: sticky; top: 0; z-index: 999; border-bottom: 1px solid rgba(255,255,255,0.1); border-radius: 0 0 20px 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.4); }
    .nav-logo { font-size: 2.2rem; font-weight: 800; background: linear-gradient(90deg, #4f46e5, #06b6d4); -webkit-background-clip: text; -webkit-text-fill-color: transparent; cursor: pointer; }
    .nav-links { display: flex; gap: 25px; align-items: center; }
    .nav-link { color: #cbd5e1; text-decoration: none; font-size: 1rem; font-weight: 600; transition: color 0.3s; }
    ...
    .glass-card { background: rgba(30, 41, 59, 0.4); border: 1px solid rgba(255, 255, 255, 0.05); border-radius: 20px; padding: 30px; transition: all 0.3s ease; }
    ...
    /* FAQ */
    details { background: rgba(255,255,255,0.03); border-radius: 12px; margin-bottom: 15px; border: 1px solid rgba(255,255,255,0.05); transition: all 0.3s; display: block; }
    details:hover { background: rgba(255,255,255,0.06); }
    details > summary { padding: 20px; color: white; font-weight: 600; font-size: 1.2rem; cursor: pointer; list-style: none; display: flex; justify-content: space-between; align-items: center; }
    details > summary::after { content: '+'; font-size: 1.5rem; color: #a855f7; transition: transform 0.3s; font-weight: 800; }
...
<div class="grid-4">
<div class="glass-card"><div class="icon-box">🧠</div><div class="card-head">Vector RAG</div><div class="card-text">Upload complex PDFs. The agent indexes every paragraph into a high-speed vector database for instant recall.</div></div>
<div class="glass-card"><div class="icon-box">👁️</div><div class="card-head">Vision Analysis</div><div class="card-text">Detects, crops, and interprets charts, graphs, and technical diagrams automatically.</div></div>
<div class="glass-card"><div class="icon-box">🌐</div><div class="card-head">Live Web</div><div class="card-text">Connects to Google Search API to verify internal data against real-time news and market trends.</div></div>
<div class="glass-card"><div class="icon-box">🛡️</div><div class="card-head">Secure Core</div><div class="card-text">Bank-grade isolation. Each session is sandboxed with strict data wiping protocols.</div></div>
</div>
</div>
...
<div class="grid-2">
<details><summary>1. Is my data secure?</summary><div class="faq-a">Absolutely. We operate on a <strong>Strict Isolation Protocol</strong>.<br>1. <strong>Session Sandbox:</strong> Every time you upload a new PDF or refresh the page, the backend completely wipes the temporary data.<br>2. <strong>No Training:</strong> Your data is processed in-memory and never used to train our models.</div></details>
<details><summary>2. Do I need an OpenAI API Key?</summary><div class="faq-a">It depends on your preference:<br>- <strong>Cloud Mode (Recommended):</strong> Yes, you need an OpenAI Key (GPT-4o) for high quality.<br>- <strong>Local Mode (Free):</strong> No, you can use Ollama (Phi-3) if you have it installed on your machine. This runs entirely offline.</div></details>
<details><summary>3. Can I upload multiple files?</summary><div class="faq-a">Currently, the Strict Mode supports <strong>one focused PDF at a time</strong>. This design choice ensures maximum accuracy and zero "context bleeding".</div></details>
<details><summary>4. What is the 'Vision' capability?</summary><div class="faq-a">Standard AI reads text, but ours sees pixels. When you enable Vision, the system scans your PDF, detects charts, and interprets them visually.</div></details>
<details><summary>5. How does Web Search work?</summary><div class="faq-a">We integrate with <strong>Serper Dev</strong>. When internal data is insufficient, the agent autonomously queries Google to verify facts.</div></details>
<details><summary>6. Is this open source?</summary><div class="faq-a">Yes! This project is built on open-source frameworks: LangChain, CrewAI, and Streamlit.</div></details>
<details><summary>7. Can I export the chat?</summary><div class="faq-a">Yes! Every response comes with a <strong>"Download Report"</strong> button generating a professional PDF.</div></details>
<details><summary>8. Does it work on mobile?</summary><div class="faq-a">Yes, the layout is fully responsive and works on smartphones and tablets.</div></details>
<details><summary>9. Who built this?</summary><div class="faq-a">This Autonomous Enterprise solution was architected and designed by <strong>Mohit</strong>.</div></details>
<details><summary>10. How do I reset the memory?</summary><div class="faq-a">Click the <strong>"Clear All History"</strong> button in the sidebar or refresh the page.</div></details>
</div>
</div>
...
<div style="text-align: center; margin-top: 50px; padding-bottom: 50px; color: #475569;">© 2025 Autonomous Enterprise Solutions • Built with ❤️ by Mohit</div>
"""
...
def get_llm(model_choice, user_api_key):
    # STRICT SECURITY: Use ONLY the key provided in the argument (from sidebar).
    # Do NOT fallback to os.getenv or st.secrets for user keys.
...
def run_simple_chat(user_question, chat_history_context, llm_instance):
    chatbot = Agent(role='Helpful Assistant', goal='Answer directly.', backstory="You are a helpful AI assistant.", llm=llm_instance)
...
def run_crew_logic(user_question, chat_history_context, llm_instance, specific_file, use_vision, use_web, target_url, serper_key):
    ...
    researcher = Agent(role='Senior Enterprise Researcher', goal=f'Gather data. {file_instr}', verbose=True, memory=True,
                       backstory=(f"You are a thorough researcher. 1. Search vector DB. 2. {vision_instr} 3. {web_instr} 4. {url_instr}"),
                       tools=researcher_tools, llm=llm_instance)
    analyst = Agent(role='Senior Data Analyst', goal='Analyze trends.', verbose=True, memory=True,
                    backstory=("Analyze data. Use 'Code Interpreter' for tables. Use 'Vision Analyst' for images."),
                    tools=[code_interpreter, vision_tool], llm=llm_instance)
    writer = Agent(role='Lead Technical Writer', goal='Write report.', verbose=True, memory=True,
                   backstory="Write professional reports.", llm=llm_instance)

    context_str = f"\nContext: {chat_history_context}" if chat_history_context else ""
    task_research = Task(description=(f"Query: '{user_question}'.{context_str}\nPDF: {specific_file}\nURL: {target_url}"), expected_output='Summary.', agent=researcher)
    task_analysis = Task(description="Analyze findings.", expected_output='Analysis.', agent=analyst, context=[task_research])
    task_writing = Task(description=f"Write answer to: '{user_question}'.", expected_output='Final report.', agent=writer, context=[task_analysis])
...
def landing_page():
    ...
    if st.session_state.auth_mode:
        ...
        with col_form:
            ...
            with tab2:
                ...
                if st.button("Create Account", use_container_width=True, type="primary"):
                    if db.register_user(new_email, new_pass, new_name): st.success("Account created! Please Login.")
        ...
        st.markdown("""<div class="nav-container"><div class="nav-logo">🤖 Autonomous Enterprise Agent</div><div class="nav-links"><a href="#features" class="nav-link">Features</a><a href="#pricing" class="nav-link">Pricing</a><a href="#faq" class="nav-link">FAQ</a><a href="#note-on-local-llms" class="nav-link">Note on Local LLMs</a><a href="#contact" class="nav-link">Contact</a></div></div>""", unsafe_allow_html=True)
        ...
        st.divider()
        st.markdown("""<div id="note-on-local-llms" style="text-align: center; max-width: 800px; margin: 0 auto; padding: 20px;"><h3 style="color: #cbd5e1;">⚠️ Note on Local Models</h3><p style="color: #94a3b8; font-size: 1.1rem;">While this Enterprise Agent supports Local LLMs (like Phi-3 & Llama 3.1), this cloud deployment does not have GPU acceleration enabled due to high server costs. To use Local Models effectively, please clone this repository and run it on your own machine.</p></div>""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    use_vision = st.checkbox("Extract Images", value=False) # Default OFF
    use_web = st.checkbox("Enable Web Search", value=False) # Default OFF
    st.divider()
...
# --- STRICT GATEKEEPING LOGIC ---
if prompt := st.chat_input("Ask your research team..."):
    # 1. BLOCK LOCAL MODELS ON CLOUD
    if model_option == "Local (Phi-3)":
        st.error("⛔ Local models are not supported on this cloud server (No GPU). Please select OpenAI.")
        st.stop()
    
    # 2. BLOCK MISSING OPENAI KEY
    if not user_api_key:
        st.warning("⚠️ Please enter your OpenAI API Key in the sidebar to proceed.")
        st.stop()

    # 3. BLOCK WEB SEARCH WITHOUT KEY
    if use_web and not serper_api_key:
        st.warning("⚠️ Web Search enabled but Serper Key missing. Please enter it in the sidebar.")
        st.stop()
