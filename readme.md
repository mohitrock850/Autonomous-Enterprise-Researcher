...
* **🔐 Enterprise Security:** Built-in **Authentication System** with hashed passwords, session isolation, and a dedicated **Admin Dashboard** for user management.
...
```mermaid
graph TD
    User[👤 User] -->|Login/Auth| DB[(SQLite Database)]
    User -->|Uploads PDF / Prompts| UI[💻 Streamlit UI]
...
```
### Workflow
1. **🔐 Login:** Use the admin credentials set in your `.env` file to access the portal.
2. **⚙️ Configure:** Enter API keys in the sidebar (or `.env`).
3. **🧠 Select Brain:** Choose **Cloud (GPT-4o)** for power or **Local (Phi-3)** for privacy.
4. **📎 Upload:** Drag & drop a PDF financial report or research paper.
5. **💬 Query:** Ask "Analyze the growth trend in Figure 1" or "Search web for competitors."
...
└── database.py                 # Database Logic (Auth, Admin, History)
